...
const MetaDb = @import("./fsdb/metadb.zig").MetaDb;
const ExtDb = @import("./fsdb/by_extension.zig").ByExtensionDb;
    ...
    if (db.get(path)) |entry| {
        ...
        switch (entry.kind) {
            ...
            .file => {
                ...
                stbuf.*.st_nlink = 1;
                stbuf.*.st_size = 0; //@intCast(c.off_t, entry.len());
            },
        }
        return 0;
    } else {
        ...
        if (db.get(path)) |entry| {
            switch (entry.kind) {
                .dir => {
                    ...
                    var children = db.childIterator(path);
                    while (children.next()) |child| {
                        ...
                        _ = filler(buf, basename_scratch.ptr, null, 0, out_flags);
                    }
                    return 0;
                },
                else => {},
...
pub fn hello_read(arg_path: [*c]const u8, arg_buf: [*c]u8, arg_size: usize, arg_offset: c.off_t, _: ?*c.struct_fuse_file_info) callconv(.C) c_int {
    ...
    if (db.get(path)) |entry| {
        ...
        size = 0; //entry.read(arg_buf[0..arg_size], offset);
...
pub fn main() !void {
    ...
    var extDb = ExtDb.init(arena.allocator(), testSource);
    defer extDb.deinit();

    // var tagDb = TagDb.init(arena.allocator());
    // defer tagDb.deinit();

    try mdb.mount("ext", extDb.idb());
