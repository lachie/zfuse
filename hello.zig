...
pub const pthread_barrierattr_t = extern union {
    __size: [4]u8,
    __align: c_int,
}; // ./libfuse/include/fuse_common.h:54:15: warning: struct demoted to opaque type - has bitfield
...
pub extern fn fuse_loop_cfg_convert(config: [*c]struct_fuse_loop_config, v1_conf: ?*struct_fuse_loop_config_v1) void; // ./libfuse/include/fuse_common.h:909:13: warning: struct demoted to opaque type - has bitfield
...
pub const struct_fuse_operations = extern struct {
    ...
    ioctl: ?*const fn ([*c]const u8, c_int, ?*anyopaque, ?*struct_fuse_file_info, c_uint, ?*anyopaque) callconv(.C) c_int,
...
pub var options: struct_options = @import("std").mem.zeroes(struct_options);
pub const option_spec: [5]struct_fuse_opt = [5]struct_fuse_opt{
    struct_fuse_opt{
        .templ = "--name=%s",
        .offset = @offsetOf(struct_options, "filename"),
        .value = @as(c_int, 1),
    },
    struct_fuse_opt{
        .templ = "--contents=%s",
        .offset = @offsetOf(struct_options, "contents"),
        .value = @as(c_int, 1),
    },
    struct_fuse_opt{
        .templ = "-h",
        .offset = @offsetOf(struct_options, "show_help"),
        .value = @as(c_int, 1),
    },
    struct_fuse_opt{
        .templ = "--help",
...
pub fn hello_init(arg_conn: [*c]struct_fuse_conn_info, arg_cfg: [*c]struct_fuse_config) callconv(.C) ?*anyopaque {
    var conn = arg_conn;
    var cfg = arg_cfg;
    _ = @TypeOf(conn);
    cfg.*.kernel_cache = 1;
...
pub fn hello_getattr(arg_path: [*c]const u8, arg_stbuf: [*c]struct_stat, arg_fi: ?*struct_fuse_file_info) callconv(.C) c_int {
    var path = arg_path;
    var stbuf = arg_stbuf;
    var fi = arg_fi;
    _ = @TypeOf(fi);
    var res: c_int = 0;
    _ = memset(@ptrCast(?*anyopaque, stbuf), @as(c_int, 0), @sizeOf(struct_stat));
    if (strcmp(path, "/") == @as(c_int, 0)) {
        stbuf.*.st_mode = @bitCast(__mode_t, @as(c_int, 16384) | @as(c_int, 493));
        stbuf.*.st_nlink = 2;
    } else if (strcmp(path + @bitCast(usize, @intCast(isize, @as(c_int, 1))), options.filename) == @as(c_int, 0)) {
        stbuf.*.st_mode = @bitCast(__mode_t, @as(c_int, 32768) | @as(c_int, 292));
        stbuf.*.st_nlink = 1;
        stbuf.*.st_size = @bitCast(__off_t, strlen(options.contents));
    } else {
        res = -@as(c_int, 2);
    }
    return res;
}
pub fn hello_readdir(arg_path: [*c]const u8, arg_buf: ?*anyopaque, arg_filler: fuse_fill_dir_t, arg_offset: off_t, arg_fi: ?*struct_fuse_file_info, arg_flags: enum_fuse_readdir_flags) callconv(.C) c_int {
    var path = arg_path;
    var buf = arg_buf;
    var filler = arg_filler;
    ...
    _ = @TypeOf(fi);
    _ = @TypeOf(flags);
    if (strcmp(path, "/") != @as(c_int, 0)) return -@as(c_int, 2);
    _ = filler.?(buf, ".", null, @bitCast(off_t, @as(c_long, @as(c_int, 0))), @bitCast(c_uint, @as(c_int, 0)));
    _ = filler.?(buf, "..", null, @bitCast(off_t, @as(c_long, @as(c_int, 0))), @bitCast(c_uint, @as(c_int, 0)));
    _ = filler.?(buf, options.filename, null, @bitCast(off_t, @as(c_long, @as(c_int, 0))), @bitCast(c_uint, @as(c_int, 0)));
    return 0;
}
pub fn hello_open(arg_path: [*c]const u8, arg_fi: ?*struct_fuse_file_info) callconv(.C) c_int {
    var path = arg_path;
    var fi = arg_fi;
    if (strcmp(path + @bitCast(usize, @intCast(isize, @as(c_int, 1))), options.filename) != @as(c_int, 0)) return -@as(c_int, 2);
    if ((fi.*.flags & @as(c_int, 3)) != @as(c_int, 0)) return -@as(c_int, 13);
    return 0;
}
pub fn hello_read(arg_path: [*c]const u8, arg_buf: [*c]u8, arg_size: usize, arg_offset: off_t, arg_fi: ?*struct_fuse_file_info) callconv(.C) c_int {
    var path = arg_path;
    var buf = arg_buf;
    var size = arg_size;
    var offset = arg_offset;
    ...
    if (@bitCast(c_ulong, offset) < len) {
        if ((@bitCast(c_ulong, offset) +% size) > len) {
            size = len -% @bitCast(c_ulong, offset);
        }
        _ = memcpy(@ptrCast(?*anyopaque, buf), @ptrCast(?*const anyopaque, options.contents + @bitCast(usize, @intCast(isize, offset))), size);
    } else {
        size = 0;
    }
    return @bitCast(c_int, @truncate(c_uint, size));
}
pub fn hello_mknod(arg_path: [*c]const u8, arg_mode: mode_t, arg_rdev: dev_t) callconv(.C) c_int {
    var path = arg_path;
    _ = @TypeOf(path);
    var mode = arg_mode;
    var rdev = arg_rdev;
    _ = @TypeOf(rdev);
    if ((mode & @bitCast(c_uint, @as(c_int, 61440))) != @bitCast(c_uint, @as(c_int, 32768))) return -@as(c_int, 1);
    return 0;
}
pub const hello_oper: struct_fuse_operations = struct_fuse_operations{
...
pub export fn main(arg_argc: c_int, arg_argv: [*c][*c]u8) c_int {
    ...
    var args: struct_fuse_args = struct_fuse_args{
        .argc = argc,
        .argv = argv,
    ...
    if (fuse_opt_parse(&args, @ptrCast(?*anyopaque, &options), @ptrCast([*c]const struct_fuse_opt, @alignCast(@import("std").meta.alignment([*c]const struct_fuse_opt), &option_spec)), null) == -@as(c_int, 1)) return 1;
    if (options.show_help != 0) {
        show_help(argv[@intCast(c_uint, @as(c_int, 0))]);
        _ = blk: {
            _ = @sizeOf(c_int);
            break :blk blk_1: {
                break :blk_1 if (fuse_opt_add_arg(&args, "--help") == @as(c_int, 0)) {} else {
                    __assert_fail("fuse_opt_add_arg(&args, \"--help\") == 0", "./hello.c", @bitCast(c_uint, @as(c_int, 179)), "int main(int, char **)");
                };
            };
        };
        args.argv[@intCast(c_uint, @as(c_int, 0))][@intCast(c_uint, @as(c_int, 0))] = '\x00';
    }
    ret = fuse_main_real(args.argc, args.argv, &hello_oper, @sizeOf(struct_fuse_operations), @intToPtr(?*anyopaque, @as(c_int, 0)));
...
pub const FUSE_OPT_KEY = @compileError("unable to translate C expr: unexpected token '{'"); // ./libfuse/include/fuse_opt.h:98:9
pub const FUSE_OPT_END = @compileError("unable to translate C expr: unexpected token '{'"); // ./libfuse/include/fuse_opt.h:104:9
pub const FUSE_ARGS_INIT = @compileError("unable to translate C expr: unexpected token '{'"); // ./libfuse/include/fuse_opt.h:123:9
pub const va_start = @compileError("unable to translate macro: undefined identifier `__builtin_va_start`"); // /home/lachie/.asdf/installs/zig/0.10.0/lib/include/stdarg.h:17:9
pub const va_end = @compileError("unable to translate macro: undefined identifier `__builtin_va_end`"); // /home/lachie/.asdf/installs/zig/0.10.0/lib/include/stdarg.h:18:9
pub const va_arg = @compileError("unable to translate macro: undefined identifier `__builtin_va_arg`"); // /home/lachie/.asdf/installs/zig/0.10.0/lib/include/stdarg.h:19:9
pub const __va_copy = @compileError("unable to translate macro: undefined identifier `__builtin_va_copy`"); // /home/lachie/.asdf/installs/zig/0.10.0/lib/include/stdarg.h:24:9
pub const va_copy = @compileError("unable to translate macro: undefined identifier `__builtin_va_copy`"); // /home/lachie/.asdf/installs/zig/0.10.0/lib/include/stdarg.h:27:9
...
pub const FUSE_BUFVEC_INIT = @compileError("unable to translate C expr: unexpected token '{'"); // ./libfuse/include/fuse_common.h:780:9
...
pub const fuse_main = @compileError("unable to translate C expr: unexpected token '*'"); // ./libfuse/include/fuse.h:899:9
pub const FUSE_REGISTER_MODULE = @compileError("unable to translate macro: undefined identifier `fuse_module_`"); // ./libfuse/include/fuse.h:1294:9
...
pub const offsetof = @compileError("unable to translate macro: undefined identifier `__builtin_offsetof`"); // /home/lachie/.asdf/installs/zig/0.10.0/lib/include/stddef.h:104:9
...
pub const OPTION = @compileError("unable to translate C expr: unexpected token '{'"); // ./hello.c:45:9
...
pub const FUSE_MINOR_VERSION = @as(c_int, 12);
